 ...
 * @description A modal component with a form for editing an existing product's details.
 */
 ...
 */
interface EditProductModalProps {
    /** The existing product data to pre-fill the form. */
    product: Product;
    /** Callback function to close the modal. */
    onClose: () => void;
    /**
     * Callback function triggered on form submission with the updated product data.
     * @param {Partial<Product>} updatedProductData - The updated product data for the backend.
     */
...
const EditProductModal: React.FC<EditProductModalProps> = ({ product, onClose, onUpdate }) => {
    ...
    const [version, setVersion] = useState(product.version || '0.1.0');
    const [stage, setStage] = useState<ProductStage>(product.stage || ProductStage.CONCEPT);
    const [customerSegment, setCustomerSegment] = useState(product.customer_segment || '');
    const [uniqueValueProp, setUniqueValueProp] = useState(product.unique_value_prop || '');
    const [targetedLaunchDate, setTargetedLaunchDate] = useState(product.targeted_launch_date || '');
