...
def update_task_scopes_raw_sql():
    """
    Connects to the database and uses a raw SQL UPDATE statement to
    convert all 'scope' values in the 'tasks' table to uppercase.
    This bypasses the SQLAlchemy Enum validation which fails on read.
    """
    app = create_app(Config)
    with app.app_context():
        print("--- Starting Task Scope Update using Raw SQL ---")
        
        try:
            # Using text() to ensure the SQL is safely handled by SQLAlchemy
            update_statement = text("UPDATE tasks SET scope = UPPER(scope)")
            
            result = db.session.execute(update_statement)
            db.session.commit()
            
            print(f"\nSuccessfully updated {result.rowcount} task scope(s).")
